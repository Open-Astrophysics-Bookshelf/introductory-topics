% !TEX TS-program = pdflatex
% !TEX useTabsWithFiles
% !TEX tabbedFile{coordinates/coordinates.tex}
% !TEX tabbedFile{light-telescopes/light-telescopes.tex}
% !TEX tabbedFile{spectroscopy/spectroscopy.tex}
% !TEX tabbedFile{detection-exoplanets/detection-exoplanets.tex}
% !TEX tabbedFile{beyond-kepler/beyond-kepler.tex}
% !TEX tabbedFile{planetary-atmospheres/planetary-atmospheres.tex}
% !TEX tabbedFile{constants-units/constants-units.tex}
% !TEX tabbedFile{math-review/math-review.tex}
% !TEX tabbedFile{math-review/trigonometry.tex}
% !TEX tabbedFile{statistics/statistics.tex}

\documentclass[symmetric]{astro-bookshelf}
\hypersetup{colorlinks=true,linkcolor=blue,citecolor=black,urlcolor=blue}

\usepackage{wasysym}
\usepackage{fontawesome5}

\graphicspath{{frontmatter/}{coordinates/figs/}{light-telescopes/figs/}{spectroscopy/figs/}{detection-exoplanets/figs/}{beyond-kepler/figs/}{planetary-atmospheres/figs/}{constants-units/figs/}{math-review/figs/}{statistics/figs/}}

\setcounter{secnumdepth}{1}

\usepackage[units,derivatives,vectors,code,symbols]{starType}
\input{symbols}
\newcommand{\newterm}[1]{\textsc{#1}}
\newcommand{\notebook}{\faLaptopCode}

% Following hack allows us to give a ``series'' name while preserving the section headers
\title[Planets and Telescopes]{{\Large Introductory Topics in Astronomy}\\Planets and\\Telescopes}
\author{Edward Brown}
\publisher{Open Astrophysics Bookshelf}
\date{28 May 2015}

\begin{document}
\frontmatter
\pagenumbering{roman}
\input{frontmatter/frontmatter}

\mainmatter
\pagenumbering{arabic}
\setcounter{page}{1}

\chapter{Coordinates: Specifying Locations on the Sky}\label{ch.coordinates}
\input{coordinates/coordinates}

\chapter{Light and Telescopes}\label{ch.light-telescopes}
\input{light-telescopes/light-telescopes}

\chapter{Spectroscopy}\label{ch.spectroscopy}
\input{spectroscopy/spectroscopy}

\chapter{Detection of Exoplanets}\label{ch.detection-exoplanets}
\input{detection-exoplanets/detection-exoplanets}

\chapter{Beyond Kepler's Laws}
\input{beyond-kepler/beyond-kepler}

\chapter{Planetary Atmospheres}
\input{planetary-atmospheres/planetary-atmospheres}

\appendix
\chapter{Constants and Units}
\input{constants-units/constants-units}

\chapter{Mathematics Review}\label{ch.math-review}
\input{math-review/math-review}

\chapter{Probability and Statistics}\label{ch.probability-statistics}
\input{statistics/statistics}

\backmatter
\bibliographystyle{plainnat}
\bibliography{bibs/AST208}

\end{document}

%\chapter{Resonances in the Solar System}
%\input{resonance/resonance}
