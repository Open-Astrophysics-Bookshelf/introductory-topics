...
A consequence of $c$ being a constant, however, is that time passes at different rates for the emitter and receiver.  This means the time $T$ in the above expression, which is in period in the source frame, is actually $T/\sqrt{1-v^{2}/c^{2}}$ in the observer's reference frame.

Finally, there is one further adjustment: in the coordinate system of observational astronomy (see chapter~\ref{ch.coordinates}) a positive velocity would be away from us. In this case, we replace $v$ in the above example with $-v$. When these changes are made, the formula for the wavelength observed from a source moving at speed $v$ is
\begin{equation}\label{e.doppler}
\lambda_{\mathrm{obs}} = \lambda_{\mathrm{source}} \left[\frac{1+v/c}{\sqrt{1-v^{2}/c^{2}}}\right].
\end{equation}
In this equation, $v$ is the velocity of the source along the line of sight; $v$ is positive if directed away from the observer.
...
\item If a motorist is going \val{40}{\meter/\second} (about 89~miles/hour) toward the officer, what is $\Delta\lambda = \lambda_{\mathrm{motorist}} - \lambda_{\mathrm{officer}}$? What is $\Delta\lambda/\lambda$?
\end{enumerate}
\end{exercisebox}
